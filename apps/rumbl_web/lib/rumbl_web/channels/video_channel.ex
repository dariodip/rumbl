defmodule RumblWeb.VideoChannel do
  ...
  def handle_in("new_annotation", params, user, %{assigns: assigns} = socket) do
    case Multimedia.annotate_video(user, assigns.video_id, params) do
      {:ok, annotation} ->
        broadcast_annotation(socket, user, annotation)
        _ = Task.start_link(fn -> compute_additional_info(annotation, socket) end)
        {:reply, :ok, socket}
